...
:::note[Safety Ratings Availability]
Safety ratings from the Gemini API may be included in the response under certain conditions (such as when content is flagged or blocked). When present, they can be accessed from the raw API response:
...
// Access the full Gemini API response
const rawResponse = response.raw as any;

// Safety ratings may be in candidates (per-response ratings)
const candidateSafetyRatings = rawResponse?.candidates?.[0]?.safetyRatings;

// Or in promptFeedback (if the prompt itself was flagged)
const promptSafetyRatings = rawResponse?.promptFeedback?.safetyRatings;
```

Note that safety ratings are not always present in the response - they are typically only included when content is flagged for safety concerns or when explicitly requested by the API.

For more information about safety settings and ratings, see the [Google AI safety settings documentation](https://ai.google.dev/gemini-api/docs/safety-settings).
:::
...
When Google Search grounding is enabled, the response includes detailed metadata about how search results were used. You can access this metadata from the response's custom metadata:
...
// Access grounding metadata from the response custom metadata
...
- **`webSearchQueries`**: Array of search queries used to retrieve information
- **`searchEntryPoint`**: Contains the main search result content with `renderedContent`
- **`groundingSupports`**: Details about how specific response segments are supported by search results, including:
...
{
  "groundingMetadata": {
    "webSearchQueries": ["best practices training large language models"],
    "searchEntryPoint": {
      "renderedContent": "..."
    },
    "groundingSupports": [
      {
        "segment": {
          "startIndex": 0,
          "endIndex": 89,
          "text": "Training large language models requires careful attention to data quality and diversity."
        },
...
:::note
For additional details on file management operations (get, list, delete), file lifecycle (48-hour automatic deletion), and usage limits (20 GB per project, 2 GB per file), see the [Google AI Files API documentation](https://ai.google.dev/gemini-api/docs/files).
:::
